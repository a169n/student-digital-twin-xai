...
def build_benchmark_diagnostics(
    ...
    interpretation = _interpret_engagement_result(fixed_model_classification, config)
    return {
...
def _interpret_engagement_result(
    ...
    if primary_delta_f1 is None:
        ...
        interpretation = (
            ...
            "KU Leuven. The additional session, content-type, forum, and temporal "
            "features provide signal beyond raw click volume and active days."
        )
        ...
        interpretation = (
            ...
            "the minimal baseline on KU Leuven. The dominant PASSED signal is already "
            "captured by cumulative clicks and active days; the additional features "
            "do not materially shift classification accuracy."
        )
    else:
        ...
        interpretation = (
            ...
            "compared with the minimal baseline on KU Leuven. This may reflect "
            "correlation noise or overfitting in the richer set."
        )
    return {
        "outcome": outcome,
...
def render_experiment_markdown(
    *,
    ...
    lines: list[str] = [
        ...
        "## Dataset",
        "",
        f"- Institution: KU Leuven (year 1819) — THIRD institution",
        f"- Citation: Tiukhova, E., Van Landuyt, D., Baesens, B., & Snoeck, M. (2026). "
...
def _validate_feature_columns(
    ...
    if missing_by_set:
        raise ValueError(f"KU Leuven feature columns are missing: {missing_by_set}")
