...
**Key constraint found:** `train_regression_reference` (explainability.py L64-151) and `build_global_explanation` (L154-238) are **already dataset-agnostic** — they take a `frame`, a `feature_set_name` (looked up via `get_feature_set`), `drop_columns`, etc., and the global explanation hardcodes only the label `"split_strategy": "student_group"` (L217). The OULAD feature sets are not in the synthetic `_REGISTRY`, so the runner must pass OULAD `BenchmarkFeatureSet`s through a small adapter rather than `get_feature_set`.
