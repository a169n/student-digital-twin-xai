...
% ANONYMITY: \blindtrue below collapses the author block to one anonymous line
% and replaces the repository URL with a note; it is currently \blindtrue for
% the double-blind revision. Switch to \blindfalse for the camera-ready version,
% and rebuild both the PDF and the .docx (the builder reads the same flag).
%
...
\newif\ifblind
\blindtrue  % \blindfalse for the camera-ready version

\begin{document}

\title{Reference Points for Explanation-Stability Claims: A Five-Institution Student-Risk Benchmark}
...
benchmark covers 63 cohorts from five universities on three learning-management
platforms in five countries, 35{,}529 distinct students, harmonised to one
...
Second, we test on this population the reported finding that the choice of explainer matters more than the sample a model is trained on, and find the comparison has no estimator-free answer. Permutation importance and SHAP agree with each other at Kendall $\tau$ 0.410, which is above permutation's own within-cohort reference of 0.345 and below SHAP's of 0.592: the reported ordering holds under one estimator and reverses under the other. Third, a single-estimator stability study reports a conclusion conditional on an undeclared choice: measuring how much a year of retraining costs a feature ranking gives $\tau$ 0.256 under permutation importance and 0.508 under SHAP, a gap of 0.252 (95\,\% interval 0.204--0.298), while the cost itself, 0.173 and 0.129, differs by an amount whose interval covers zero, so the estimator moves the reported level by more than the effect under study. We also report two
...
either needs what most studies lack: many institutions, or many
estimators applied to the same models.

This paper supplies both. It is a resource paper with a measurement result attached, not a methods paper: we propose no new model, explainer or transfer technique.

Contributions:

\begin{enumerate}
\item A public five-institution benchmark: 63 cohorts, 35{,}529 distinct
students, three platforms, five countries, one harmonised schema, with adapters,
...
\item A consequence: single-estimator stability studies report
conclusions conditional on an undeclared choice; we quantify how much
...
\subsection{Five institutions, three platforms, five countries}

Table~\ref{tab:cohorts} summarises the corpus and Fig.~\ref{fig:pipeline} the
pipeline that produces it. A cohort is one course, one
...
Section~\ref{sec:controls} shows is not a bookkeeping detail. Within a cohort a
student contributes exactly one row, so the folds of the within-cohort
cross-validation never share a student; repetition is a cross-cohort matter,
and Section~\ref{sec:controls} reports it per pair. Outcome definitions
...
construction. Every cross-institution number in this paper --- the
per-institution means of Table~\ref{tab:cohorts}, the variance share in
Section~\ref{sec:pred}, the bottom rung of Fig.~\ref{fig:instability} --- is
therefore reported as describing the corpus, not as an effect of
institution, and the measurement result of Sections~\ref{sec:agree}
and~\ref{sec:cond} rests only on comparisons within one cohort or one course,
where the outcome definition is fixed.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{fig0_pipeline.pdf}
\caption{The pipeline, from five public releases to the reference points. Blue stages build the benchmark; ochre stages evaluate on it.}
\label{fig:pipeline}
\end{figure}
...
\begin{table*}[t]
\caption{The benchmark. Pass rate is the range over the institution's cohorts; within-cohort ROC-AUC is the mean of 5-fold cross-validated gradient boosting over them, descriptive only because outcome definitions differ by institution.}
\label{tab:cohorts}
\centering
...
\midrule
Total & 3 platforms & 5 countries & 63 & 35{,}529 distinct & 0.25--0.95 & --- \\
\bottomrule
\end{tabular}
\end{table*}
...
enters the models; three institutions publish none. Courses run 14 to
...
twin of the learner, but the term is graded by data flow: a \emph{digital
shadow} carries an automated one-way flow from the object to its representation,
and a \emph{digital twin} closes the loop with a flow back
\cite{kritzinger2018,addanki2023}. Our schema is one-way by construction, so it
is a digital shadow, and we claim no simulation, counterfactual or intervention
capability; systems that do close the loop keep a teacher who can inspect and
override model-scored output before it reaches the student
\cite{yerniyazov2026}. The distinction is load-bearing: closing the loop means
reading feature attributions causally, which Section~\ref{sec:agree} argues
against, since the attributions move more between estimators than the effect
under study.
...
smaller benchmark, not the leaked one.
...
regression and a random forest as robustness checks (Table~\ref{tab:models}).
Nothing is tuned per cohort: the object of study is what an ordinary model does.
The explanation analyses use gradient boosting alone, since TreeSHAP needs a
tree ensemble and those analyses hold the model fixed while the
estimator varies.
...
institution, but those are means; the claim is about spread.
Fig.~\ref{fig:spread} gives one row per cohort. Within-cohort ROC-AUC runs from
0.384 to 0.953, each interval a 400-resample bootstrap over that
cohort's students. Eighteen of the 63 intervals cover 0.5, and one Oviedo course sits
entirely below it at 0.384. A study reporting 0.85 and one reporting 0.55 may
be equally competent and correct about their data.

Institution --- and with it platform, outcome definition and scale, which are collinear here --- accounts for 57.3\,\% of the variance in cohort AUC, as the between-group share of the total sum of squares in a one-way analysis of variance over the five institutions; the figure describes the corpus and is not an estimate of an institution effect. The remainder is
...
which a single-dataset design cannot observe.

The spread is not a property of gradient boosting. Table~\ref{tab:models}
repeats the within-cohort measurement with logistic regression and a random
forest: the ordering of institutions is the same under all three, the three
models sit within 0.05 of one another at every institution but Zambia, whose
two cohorts hold 117 students between them, and logistic regression is the best
of the three overall, 0.717 against 0.691 and 0.692. Transferred to another
institution, all three lose most of what they had.

\begin{table}[t]
\caption{Mean within-cohort ROC-AUC per institution for the three model families and the zero-training rule; the last row is cross-institution transfer on pairs that share no student.}
\label{tab:models}
\centering
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
 & Gradient & Logistic & Random & Rule: active \\
 & boosting & regression & forest & days \\
\midrule
Open University & 0.845 & 0.858 & 0.848 & 0.830 \\
KwaZulu-Natal & 0.667 & 0.716 & 0.673 & 0.717 \\
KU Leuven & 0.608 & 0.652 & 0.614 & 0.655 \\
Oviedo & 0.606 & 0.630 & 0.611 & 0.631 \\
Zambia & 0.527 & 0.449 & 0.418 & 0.565 \\
\midrule
All 63 cohorts & 0.691 & 0.717 & 0.692 & 0.713 \\
Another institution & 0.596 & 0.598 & 0.630 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{fig2_cohort_spread.pdf}
\caption{The 63 cohorts. (a) Size and pass rate; colour and marker identify the institution. (b) Within-cohort ROC-AUC, sorted, with 400-resample bootstrap intervals over students.}
\label{fig:spread}
\end{figure*}

\subsection{Reference points, and a replication with no estimator-free answer}\label{sec:agree}

Fig.~\ref{fig:instability} places every comparison in this paper on one axis, and the grey bars are what the others are read against. The noise floor is what one fitted model scores against itself when only the permutation seed changes, so no bar can be read as stable above it; the random baseline, $\tau$ 0.000, is the axis origin. Two models differing only in which half of one cohort they saw agree at $\tau$ 0.338 (0.279--0.398) under permutation importance: that ceiling is the most such a comparison attains under it, and every bar measured under permutation importance (blue and brown) is read against it, while the two cross-estimator bars (orange) have no single estimator's scale and are read against each estimator's own reference in Table~\ref{tab:cond}. The noise floor of 0.706 (0.669--0.742) rules out estimator variance.

Against those bounds, the estimator comparison. Applied to one fitted model on
one set of students, permutation importance and SHAP agree at $\tau$ 0.410 (0.355--0.464), permutation and drop-column at 0.259 (0.213--0.303), SHAP and drop-column at 0.127 (0.074--0.179). Taken bare, the two low numbers look like the conclusion of Swamy et al.\ \cite{swamy2022} --- that the explainer is a first-order determinant of the resulting factor list --- now on 63 cohorts across five institutions, on one gradient-boosting model rather than one recurrent network over five MOOCs. Taken against the bounds, neither is a like-for-like arm: one estimator is degenerate here, and the remaining pair has no single scale.
...
near-duplicates behind, so drop-column importance is zero or negative for 3.2 of 7 features on average, against 2.0 for permutation and 0.1 for SHAP. Its agreement with itself across resamples of one cohort is 0.173 (0.120--0.227) --- lower than its agreement with permutation on identical data, and against 0.592 (0.540--0.641) for SHAP. On collinear behavioural features, drop-column is
not a reliable estimator, and 0.127 should be read as evidence about that estimator in this setting rather than as a headline. The defensible figure is the permutation-versus-SHAP agreement of 0.410, and it does not settle the question, because a cross-estimator quantity has no single estimator's scale. Against permutation importance's own within-cohort reference of 0.345 (0.287--0.401) it is high, and the explainer matters \emph{less} than the training sample; against SHAP's 0.592 it is low, and the explainer matters \emph{more}, which is the ordering Swamy et al.\ report. The permutation-only ceiling of 0.338 is the strictest reference we have and supports the first reading, but it too is one estimator's. The comparison has no estimator-free answer here, and Section~\ref{sec:cond} measures why.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.86\textwidth]{fig1_explanation_instability.pdf}
\caption{Every agreement comparison in this paper on one scale. Colour marks what differed between the two rankings; whiskers are 95\,\% cluster-bootstrap intervals over cohorts, or over target cohorts for the three transfer bars; dotted lines mark the floor and the ceiling, and the axis origin, $\tau = 0$, is the expected agreement of two independent rankings. Starred bars involve drop-column, degenerate here (Section~\ref{sec:agree}).}
...
Each estimator needs its own reference, because they disagree on the level even when nothing is retrained. Applied twice to the cohorts in those pairs under two different splits, SHAP reproduces itself at $\tau$ 0.638 (0.595--0.678) and permutation importance at 0.429 (0.383--0.472). Against those references, a year of retraining costs 0.129 (0.091--0.167) under SHAP and 0.173 (0.123--0.224) under permutation; the paired difference, -0.043, has an interval of -0.095 to 0.009 (Wilcoxon signed-rank $p = 0.15$), so as far as these pairs can tell the two estimators see an effect of one size. The level at which that effect is reported differs by 0.252 (0.204--0.298; $p < 0.001$), 1.7 times the mean effect of 0.151. Fig.~\ref{fig:instability} shows the ladder's D1 rung at 0.210, not 0.256, because the ladder scores the whole target cohort, not a held-out third; the gap between protocols is small beside that between estimators.

\begin{table*}[t]
\caption{One question, three estimators, on identical models and evaluation sets: 78 course-year pairs, three splits each; brackets are 95\,\% bootstrap intervals over the pairs. $^{*}$Degenerate here (Section~\ref{sec:agree}).}
\label{tab:cond}
\centering
\begin{tabular}{@{}lccc@{}}
\toprule
...
\midrule
SHAP & 0.638 [0.595, 0.678] & 0.508 [0.471, 0.545] & 0.129 [0.091, 0.167] \\
Permutation importance & 0.429 [0.383, 0.472] & 0.256 [0.214, 0.298] & 0.173 [0.123, 0.224] \\
Drop-column$^{*}$ & 0.199 [0.146, 0.253] & 0.055 [0.021, 0.089] & 0.144 [0.077, 0.212] \\
\midrule
\multicolumn{3}{@{}l}{Spread of the level, SHAP $-$ permutation (paired bootstrap; Wilcoxon $p < 0.001$)} & 0.252 [0.204, 0.298] \\
\multicolumn{3}{@{}l}{Mean cost of one year, SHAP and permutation} & 0.151 \\
\multicolumn{3}{@{}l}{Cost difference, SHAP $-$ permutation (paired bootstrap; Wilcoxon $p = 0.15$)} & $-$0.043 [$-$0.095, 0.009] \\
\bottomrule
\end{tabular}
\end{table*}

A stability study that fixes one estimator therefore reports a number that is as much a property of that choice as of the data, and none we know of declares which estimator produced it. The recommendation is cheap: report agreement under at least two estimators, and what each scores when nothing has changed, since without that reference a $\tau$ has no scale.
...
default positive label, \emph{passing}. Every configuration looked
healthy while losing, on the class an alerting system acts on, to a rule that
...
The benchmark makes one thing cheap: anyone can run a new model, representation or explainer against 63 cohorts from three platforms in five countries and report our reference points.
...
\textbf{Limitations.} Seven behavioural features are strongly correlated, exactly
the setting where importance is least identifiable and in which
drop-column degenerates; richer feature sets may behave differently and we cannot
test that where assessment data is unpublished. Outcome semantics differ across institutions and are collinear with
institution. Three institutions' course calendars are inferred, not published. The $\tfrac{1}{3}$ cutoff is not load-bearing: at $\tfrac{1}{4}$ and $\tfrac{1}{2}$ every level moves and no ordering does, with cross-institution explanation agreement 0.038, 0.034 and 0.029 at the three cutoffs and accuracy rising with the later one.
The intervals on agreement figures resample cohorts, or target cohorts, and so price the sampling of cohorts, not of students within them; the estimator spread in Section~\ref{sec:cond} rests on 78 course-year pairs, three splits each, from 16 course families at four institutions, and pairs from one family are not independent, so its interval is narrower than a family-clustered one would be. Cross-institution comparisons are descriptive throughout, for the reason given in Section~\ref{sec:data}. Everything here is correlational, and Section~\ref{sec:agree} is an
argument against reading any of these rankings causally.

\section{Conclusion}

We release a benchmark of 63 student cohorts from five universities, three
platforms and five countries, harmonised to one schema, with adapters and frozen
results. Within-cohort ROC-AUC spans 0.384 to 0.953, so a single-dataset result is a statement about its dataset. We supply the floor, ceiling and random baseline an explanation-agreement number needs to be read at all, and find the reported dominance of the explainer over the training sample holds or reverses according to which estimator supplies the reference. The consequence we most want read is narrow: a stability number reported with one estimator and no same-data reference has a level that moves, when the estimator changes, by more than the effect under study. Reporting two, and what each scores when nothing has changed, costs one run and makes the answer mean something.

\section*{Reproducibility}

We release the five institution adapters, the cohort definitions, the experiment code and the frozen result files, together with a script that recomputes every number in this paper from those files. We redistribute none of the student data. All five releases are published openly by their originating institutions, four under a DOI; the repository records each source, licence and expected path, and the adapters read a copy the reuser downloads. \ifblind An anonymised repository is linked from the submission record; the permanent link replaces this note in the camera-ready version.\else Everything named here is at \url{https://github.com/a169n/student-risk-benchmark}.\fi

\section*{Ethics}

All five datasets are published in de-identified form by their originating institutions, and this is a secondary analysis of those releases: no student was contacted and no re-identification was attempted. No demographic attribute enters any model. One release ships a name column beside its hashed student identifier; our adapter never reads it and joins on the hash alone, and we note it so other reusers choose deliberately.
