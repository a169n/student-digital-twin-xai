...
This paper supplies both and reports what they show. It is a resource paper with a measurement result attached, not a methods paper: we propose no new model, explainer or transfer technique.
...
therefore reported as a description of the corpus, not as an effect of
...
where the outcome definition is held fixed.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig0_pipeline.pdf}
...
The explanation analyses use the gradient-boosting model alone, since TreeSHAP is
defined for tree ensembles and those analyses hold the model fixed while the
estimator varies.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig2_cohort_spread.pdf}
\caption{The 63 cohorts. (a) Size and pass rate; colour and marker shape identify the institution. (b) Within-cohort ROC-AUC, sorted, with 400-resample bootstrap intervals over students.}
\label{fig:spread}
\end{figure*}

\subsection{Reference points, and a replication with no estimator-free answer}\label{sec:agree}

Fig.~\ref{fig:instability} places every comparison in this paper on one axis, and the grey bars are what the others are read against. The noise floor is what one fitted model scores against itself when only the permutation seed changes, so no bar can be read as stable above it; the random baseline, $\tau$ 0.000, is the axis origin. Two models differing only in which half of one cohort they saw agree at $\tau$ 0.338 (0.279--0.398) under permutation importance: that ceiling is the most such a comparison attains under that estimator, and every bar measured under permutation importance (blue and brown) is read against it, while the two cross-estimator bars (orange) have no single estimator's scale and are read against each estimator's own reference in Table~\ref{tab:cond}. The noise floor of 0.706 (0.669--0.742) confirms this is not estimator variance.
...
The benchmark's value is in what it makes cheap: any group can now run a proposed model, representation or explainer against 63 cohorts from three platforms in five countries and report the same reference points we do.
...
institution. Course calendars for three institutions are inferred rather than published. The $\tfrac{1}{3}$ cutoff is not load-bearing: at $\tfrac{1}{4}$ and $\tfrac{1}{2}$ every level moves and no ordering does, with cross-institution explanation agreement 0.038, 0.034 and 0.029 at the three cutoffs and accuracy rising with the later one.
The intervals on agreement figures resample cohorts, or target cohorts, and so price the sampling of cohorts but not the sampling of students within them; the estimator spread in Section~\ref{sec:cond} rests on 78 course-year pairs, three splits each, from 16 course families at four institutions, and pairs from one family are not independent, so its interval is narrower than a family-clustered one would be. Cross-institution comparisons are descriptive throughout, for the reason given in Section~\ref{sec:data}. Everything here is correlational, and Section~\ref{sec:agree} is an
...
All five datasets are published in de-identified form by their originating institutions, and this work is a secondary analysis of those public releases: no student was contacted and no re-identification was attempted. No demographic attribute enters any model. One release ships a name column beside its hashed student identifier; our adapter never reads it and joins on the hash alone, and we note it so that other reusers make that choice deliberately.
